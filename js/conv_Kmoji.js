/*-----------------------------------------------------------------------
★環境依存文字を数値文字参照に変換★
１）変数「kmoji」にホワイトリストとして使える文字を定義
２）「JIS X 0201」&「JIS X 0208」をホワイトリストの文字として登録
３）textareaに入力された文字と「kmoji」の文字を１文字づつ比較して、
　　環境依存文字かどうかを確認
４）環境依存文字は数値文字列に変換する
５）数値文字参照に置き換えられた文字は背景赤で表示
-----------------------------------------------------------------------*/
function conv_Kmoji(){
alert("環境依存文字を数値文字列に参照します");
	var kmoji = new Array(
...
/*改行コードを削除*/
	...
	num = '<div style="color:blue;font-weight:bold;">■機種依存文字を日経の変換ルールか、数値文字参照に置き換えました。</div>';
